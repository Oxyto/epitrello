<script lang="ts">
	...
	import UserAdminManagementSection from './UserAdminManagementSection.svelte';
	...
	onMount(() => {
		...
		const initialDisplayName = data.name?.trim() || currentUser.name?.trim() || data.email.split('@')[0];
		displayName = initialDisplayName;
		ready = true;
	});

	function updateLocalUserName(name: string) {
		if (!browser) return;
	...
	>
		...
		>
			<div class="mb-5 flex flex-wrap items-start justify-between gap-3">
				<div>
					<p class="text-xs font-semibold uppercase tracking-[0.2em] text-sky-200/90">User Settings</p>
		...
		{#if isAdmin}
			<UserAdminManagementSection {currentUserId} currentSettingsUserId={data.user_id} />
		{/if}
